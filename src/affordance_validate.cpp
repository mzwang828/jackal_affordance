...
class AffordanceValidate
{
  public:
                                                              ...
                                                              action_name_(name)
    {
        ...
        nh_.getParam("push_force_max", push_force_max_);
        nh_.getParam("lift_force_max", lift_force_max_);
    ...
    bool move_arm(float joint_state[5], float force_limit)
    {
        TrajectoryClient tc(arm_trajectory_topic_, true);
        tc.waitForServer();
        control_msgs::FollowJointTrajectoryGoal goal;
        ...
        ros::Time endTime = ros::Time::now() + ros::Duration(10);

        while (ros::Time::now() < endTime && force_[0] < force_limit)
        {
            // ROS_INFO("force value %f", force_[0]);
            if (tc.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
                return true;
        }
        tc.cancelGoal();
    ...
    void executeCB(const jackal_affordance::ValidateGoalConstPtr &goal)
    {
        ...
        if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
        {
            ...
            case 2:
            {
                ...
                ros::Duration(0.5).sleep();
                geometry_msgs::Twist tw;
                // slightly adjust orientation to align gripper and object
                tw.angular.z = -0.07;
                ...
                if (!this->move_arm(joint_state, lift_force_max_))
                {
                    ROS_INFO("Validation Failed, Non-Liftable Object");
                    result_ = 1;
                }
                ...
                if (result_ == 1)
                {
                    gc_.release();
                    tw.linear.x = -0.1;
                    ros::Time endTime = ros::Time::now() + ros::Duration(2);
                    while (ros::Time::now() < endTime)
                    {
                        velocity_pub_.publish(tw);
                    }
  ...
  private:
    ...
    int result_ = 0; // 0-nonmovable, 1-pushable, 2-liftable
    int push_force_max_, force_min_, lift_force_max_;
